...
interface QueryPageListProps<TAppRoute extends AppRoute, TClientArgs extends ClientArgs> {
  ...
  renderItem: (item: { data: RouteData<TAppRoute, TClientArgs>, index: number }) => ReactNode
      ...
      return (
        <>
          {cloneElement(
            Wrapper,
            {},
            processed.map((_item: any, index: number) => {
              const element = renderItem({ data: _item, index })
              // Ensure each element has a key prop
              if (React.isValidElement(element)) {
                const itemId = (_item as any)?.id
                // If element already has a key, return it as is
                if (element.key !== null && element.key !== undefined) {
                  return element
                }
                // If element doesn't have key, clone it with a key
                return React.cloneElement(element, { key: itemId || index })
              }
              return element
            }),
          )}

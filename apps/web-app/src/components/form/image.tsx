...
function ImageChooser({
  ...
  emptyMessage = 'Click to upload image',
}: ImageChooseProps & {
  image: string
              ...
              ? (
                  <div className="flex size-full px-4 py-8 flex-col items-center justify-center rounded border border-dashed border-gray-400">
                    ...
                    <div className="mt-2 text-xs text-gray-400">
                      Supports
                      {' '}
                      {limitType.join(', ').toUpperCase()}
                      {' '}
                      format, max
                      {' '}
                      {Math.round(limitSize / 1024 / 1024)}
                      MB (large files will be auto-compressed)
                    </div>
                  </div>
                )
              : (

...
/** Deploy vars: always taken from apps/web-app/.env so shell exports cannot override wrong IP/port. */
...
async function applyDeployEnvFromDotenv() {
  ...
  for (const line of text.split('\n')) {
    ...
    if (!DEPLOY_KEYS.includes(key as (typeof DEPLOY_KEYS)[number]))
      continue
...
const remotePath = '/var/www/neziva-www'

if (!host) {
  throw new Error('SERVER_IP is missing in apps/web-app/.env')
}

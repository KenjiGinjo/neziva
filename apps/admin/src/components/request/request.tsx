...
const Handler = observer((props: RequestProps) => {
  ...
  const onClick: MouseEventHandler = props.onClick
    ...
    : async (e: MouseEvent) => {
      ...
      if (showModal) {
        try {
          const confirmed = await showModalBase({
            title: showModalOption?.title || '系统提示',
            description: showModalOption?.description || '您确定要继续吗？',
            showCancel: true,
            confirmText: '确认',
            cancelText: '取消',
          })
      ...
      setLoading(true)
      if (showLoading) {
        loadingToastId = toast.loading(showLoadingOption?.title || '加载中...', { description: showLoadingOption?.description || '', position: 'top-center' })
      }
